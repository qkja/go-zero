...
import (
	...
	"github.com/qkja/go-zero/core/mapping"
	"github.com/qkja/go-zero/internal/encoding"
)
...
var (
	...
	fillDefaultIfZeroUnmarshaler = mapping.NewUnmarshaler(jsonTagKey, mapping.WithDefaultIfZero())
...
// FillDefaultIfZero fills the default values for v only for fields that are
// currently zero, keeping already-loaded values intact.
// It is used for configs partially loaded by other loaders (e.g. gobase),
// where default tag values like `json:",default=X"` are not applied.
func FillDefaultIfZero(v any) error {
	return fillDefaultIfZeroUnmarshaler.Unmarshal(map[string]any{}, v)
}
