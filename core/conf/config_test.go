...
import (
	...
	"github.com/qkja/go-zero/core/fs"
...
func TestFillDefaultIfZero(t *testing.T) {
	t.Run("fills only zero fields", func(t *testing.T) {
		var val struct {
			Name    string `json:",default=default-name"`
			Host    string `json:",default=0.0.0.0"`
			Port    int    `json:",default=8888"`
			Timeout int64  `json:",default=3000"`
			Mode    string `json:",default=pro,options=dev|test|rt|pre|pro"`
		}
		val.Name = "already-set"

		assert.Nil(t, FillDefaultIfZero(&val))
		assert.Equal(t, "already-set", val.Name) // 已加载的值不被覆盖
		assert.Equal(t, "0.0.0.0", val.Host)     // 零值字段被补齐默认值
		assert.Equal(t, 8888, val.Port)
		assert.Equal(t, int64(3000), val.Timeout)
		assert.Equal(t, "pro", val.Mode)
	})

	t.Run("recurses into zero nested struct", func(t *testing.T) {
		type Inner struct {
		...
		assert.Nil(t, FillDefaultIfZero(&val))
		assert.False(t, val.Enabled)
		assert.True(t, val.Middle.Trace)
		assert.True(t, val.Middle.Log)
	})
}
